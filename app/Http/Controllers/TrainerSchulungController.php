...
class TrainerSchulungController extends Controller
{
    public function index()
    {
        $modules = $this->trainerModules()
            ->with(['skillCategory', 'method', 'trainingMaterials', 'careerLevel.careerPath'])
            ->withCount(['trainingSessions', 'enrollments'])
            ->orderBy('title')
            ->get();

        return view('trainer.schulungen', compact('modules'));
    }
    ...
    public function show(Module $module)
    {
        ...
        $module->load(['skillCategory', 'method', 'trainingMaterials.uploader', 'trainingSessions.enrollments']);
    ...
    public function update(Request $request, Module $module)
    {
        ...
        $module->update($request->only('description'));
